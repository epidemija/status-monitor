...
async function geoCheckSite(site) {
  ...
  // Nodes need ~10 seconds to complete their probes.
  await sleep(12000);
  ...
  const rows = LOCATIONS.map((loc) => {
    ...
    // null = still processing or node unavailable
    if (raw == null) return [site.id, loc.key, loc.label, null, null, 0, 'No data from probe'];
    // check-host.net returns [[status, httpCode, responseTimeSec, message]]
    const check = Array.isArray(raw) && Array.isArray(raw[0]) ? raw[0] : null;
    if (!check) return [site.id, loc.key, loc.label, null, null, 0, 'Probe returned no data'];
    const [status, httpCode, rtSec, msg] = check;
    const isUp = status === 1 && httpCode != null && httpCode >= 200 && httpCode < 400 ? 1 : 0;
    const rtMs = rtSec != null ? Math.round(rtSec * 1000) : null;
    const error = status !== 1 ? (typeof msg === 'string' ? msg : 'Connection failed') : null;
    return [site.id, loc.key, loc.label, rtMs, httpCode, isUp, error];
  });
