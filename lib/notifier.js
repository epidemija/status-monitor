...
function isSlackEnabled() {
  return getSetting('slack_enabled') === '1' && !!getSetting('slack_webhook_url');
}

function isDiscordEnabled() {
  return getSetting('discord_enabled') === '1' && !!getSetting('discord_webhook_url');
}
...
async function sendEmail(subject, body) {
  ...
  // Send one email to all recipients in a single SMTP transaction.
  // Looping sendMail calls re-open the connection each time, which many
  // SMTP servers reject after the first send.
  const to = recipients.join(', ');
  try {
    await transporter.sendMail({ from, to, subject, text: body });
    console.log(`[notifier] email sent to ${to}`);
    return { results: recipients.map(r => ({ to: r, ok: true })) };
  } catch (err) {
    console.error(`[notifier] email FAILED:`, err.message);
    return { results: recipients.map(r => ({ to: r, ok: false, error: err.message })) };
  }
}
...
// ----- Microsoft Teams via Power Automate Workflow webhook -----
// The webhook is a Power Automate flow that reads triggerBody()?['text'] and
// posts it to a Teams channel. Send exactly {"text": "..."} — nothing else.
...
async function sendSlack(text) {
  ...
  try {
    await axios.post(url, { text }, { headers: { 'Content-Type': 'application/json' }, timeout: 10000 });
    return { ok: true };
  } catch (err) {
...
async function sendDiscord(content) {
  ...
  if (!url) return { ok: false, error: 'No Discord webhook URL' };
  try {
...
async function sendTestSlack() {
  const url = getSetting('slack_webhook_url');
  if (!url) return { ok: false, error: 'No Slack webhook URL configured in Settings' };
  return sendSlack(`✅ *Test — Status Monitor*\nSlack notifications are configured correctly.\nSent at: ${new Date().toISOString()}`);
}

async function sendTestDiscord() {
  const url = getSetting('discord_webhook_url');
  if (!url) return { ok: false, error: 'No Discord webhook URL configured in Settings' };
  return sendDiscord(`✅ **Test — Status Monitor**\nDiscord notifications are configured correctly.\nSent at: ${new Date().toISOString()}`);
}
...
async function sendDownAlert(site, result) {
  ...
  if (isSlackEnabled()) {
    if (withinCooldown(site.id, 'slack')) {
      console.log(`[notifier] slack cooldown active for ${site.name} — skipping`);
    } else {
      const out = await sendSlack(body);
      logNotification(site.id, 'slack', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
    }
  }
  if (isDiscordEnabled()) {
    if (withinCooldown(site.id, 'discord')) {
      console.log(`[notifier] discord cooldown active for ${site.name} — skipping`);
    } else {
      const out = await sendDiscord(body);
      logNotification(site.id, 'discord', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
    }
  }
}
...
async function sendSslWarning(site, result, thresholdDays) {
  ...
  if (isTeamsEnabled()) {
    const out = await sendTeamsCard(`⚠️ SSL expiring ≤${thresholdDays}d: ${site.name}`, body, 'B08800');
    logNotification(site.id, 'teams-ssl', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
  }
  if (isSlackEnabled()) {
    const out = await sendSlack(body);
    logNotification(site.id, 'slack-ssl', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
  }
  if (isDiscordEnabled()) {
    const out = await sendDiscord(body);
    logNotification(site.id, 'discord-ssl', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
  }
}
...
async function sendUpAlert(site) {
  ...
  if (isTeamsEnabled()) {
    const out = await sendTeamsCard(`🟢 RECOVERED: ${site.name}`, body, '2DA44E');
    logNotification(site.id, 'teams', 'webhook', out.ok ? 'ok' : 'error', out.ok ? null : out.error);
  }
  if (isSlackEnabled()) {
    if (withinCooldown(site.id, 'slack')) {
