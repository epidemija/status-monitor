...
async function checkSite(site) {
  ...
  if (parsedUrl.protocol === 'https:') {
    const ssl = await checkSsl(parsedUrl.hostname, parsedUrl.port || 443);
  ...
  // 2) HTTP request with redirect tracking and timing
  let redirectCount = 0;
  ...
  try {
    ...
    const sslOk = parsedUrl.protocol !== 'https:' || result.ssl_valid === 1;
    ...
    if (!result.is_up && !result.error_message) {
      if (!statusOk) result.error_message = `Unexpected status ${response.status}`;
    ...
    result.error_message = (result.error_message ? result.error_message + ' | ' : '') +
      (err.code || err.message || 'Request failed');
  }
...
async function maintainIncident(site, result) {
  ...
  // Down alert: fires when the incident first opens AND on every subsequent
  // check while the site is still down. The cooldown inside sendDownAlert
  // (default 60 min) prevents more than one message per hour per channel.
